        ...
        <div class="intro-y col-span-12 lg:col-span-9">
            <div class="post intro-y overflow-hidden box">
                <div class="post__tabs nav nav-tabs flex-col sm:flex-row bg-gray-300 dark:bg-dark-2 text-gray-600"
                    ...
                    <a wire:click="setTabActive('tabProducts')" title="Productos Agregados" data-toggle="tab"
                        data-target="#tabProducts" href="javascript:;"
                    ...
                    <a wire:click="setTabActive('tabCategories')" title="Seleccionar Categoría" data-toggle="tab"
                        data-target="#tabCategory" href="javascript:;"
                                    ...
                                    <table class="table">
                                        <thead>
                                            <tr class="text-theme-6">
                                                <th class="border-b-2 dark:border-dark-5 whitespace-nowrap font-bold"
                                                    width="60%">DESCRIPCIÓN</th>
                                                <th class="border-b-2 dark:border-dark-5 whitespace-nowrap font-bold">
                                                    PRECIO</th>
                                                <th class="border-b-2 dark:border-dark-5 whitespace-nowrap font-bold text-center"
                                                    width="17%">CANT</th>
                                                <th class="border-b-2 dark:border-dark-5 whitespace-nowrap font-bold">
                                                    IMPORTE</th>
                                                <th class="border-b-2 dark:border-dark-5 whitespace-nowrap font-bold">
                                        ...
                                        <tbody>
                                            @forelse($contentCart as $item)
                                                <tr class="bg-gray-200 dark:bg-dark-1 text-lg">
                                                    <td class="border-b dark:border-dark-5 ">
                                                        <button
                                                            onclick="openModal({{ $item->id }},'{{ $item->changes }}','{{ $item->name }}')"
                                                            class="btn btn-outline-secondary text-theme-1">{{ $item->name }}</button>
                                                        <div>
                                                    ...
                                                    <td class="border-b dark:border-dark-5 text-center">
                                                        <div class="input-group mt-2">
                                                            <input
                                                                ...
                                                                value="{{ $item->qty }}"
                                                                data-kioskboard-type="numpad" data-type="qty"
                                                                type="text"
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="text-center">AGREGA PRODUCTOS AL CARRITO
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                        ...
                        <div class="intro-y grid grid-cols-12 gap-3 sm:gap-4 mt-2">
                            ...
                            @if (!$showListProducts)
                                @foreach ($categories as $category)
                                    <div class="intro-y col-span-6 sm:col-span-4 md:col-span-3 xxl:col-span-2">
                                        <div wire:click="getProductsByCategory({{ $category->id }})"
                                            ...
                                            <a href="javascript:;"
                                                class="hidden block font-medium mt-4 text-center truncate">{{ $category->name }}</a>
                            ...
                            @else
                                @forelse($productsList as $product)
                                    <div class="intro-y col-span-6 sm:col-span-4 md:col-span-3 xxl:col-span-2">
                                        <div wire:click="add2Cart({{ $product->id }})"
                                            class="file box rounded-md px-5 pt-8 pb-5 px-3 sm:px-5 relative zoom-in">
                                            ...
                                            <a href="javascript:;"
                                                class="block font-medium mt-4 text-center truncate">{{ $product->name }}</a>
                                            <h1 class="text-center">${{ number_format($product->price, 2) }}</h1>
                                        </div>
                                    </div>
                                @empty
                                    <div class="col-span-12">
                                        <h1 class="text-center text-theme-6 w-full">No hay productos en la categoría
                                            seleccionada</h1>
                                    </div>
                                @endforelse
                            @endif
        ...
        <div class="col-span-12 lg:col-span-3">
            <div class="intro-y box p-5">
                <div>
                    ...
                    <button onclick="openModalCustomer()"
                        class="btn btn-outline-dark w-full mb-3">{{ $customerSelected }}</button>
                </div>
                <div class="mt-3">
                ...
                <div class="mt-6">
                    <div class="input-group">
                        <div id="input-group-3" class="input-group-text"><i class="fas fa-dollar-sign fa-2x"></i>
                        </div>
                ...
                <div class="mt-8">
                    ...
                    @if ($totalCart > 0 && $cash >= $totalCart)
                        ...
                        <button wire:loading.attr="disabled" wire:target="storeSale"
                            wire:click.prevent="storeSale(true)" class="btn btn-outline-primary w-full mt-5"><i
                                class="fas fa-receipt mr-2"></i> Guardar e Imprimir</button>
                    @endif
                    <br>
                    @if ($totalCart > 0)
                        <button onclick="Cancel()" class="btn btn-danger w-full mt-5">
                            <i class="fas fa-trash mr-2"> </i>Cancelar Venta</button>
                    @endif
                    <br>
                    <button wire:loading.attr="disabled" wire:target="storeSale" wire:click.prevent="storeSale"
                        class="btn btn-primary w-full mt-5"><i class="fas fa-database mr-2"></i> Guardar
                        Venta</button>
                </div>
            </div>
        </div>
